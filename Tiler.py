...
# Initialize this global flag at the module level
_PROFILER_RUNNING = False

def profile_operator(func):
    """Decorator to profile a Blender operator's execute method safely."""
    def wrapper(self, context, *args, **kwargs):
        global _PROFILER_RUNNING
        
        # If already profiling, just execute and return
...
def set_tile_edge_attributes(bm, depth, attr_names):
    ...
    if l_normal:
        for f in bm.faces:
            ...
            # Use raw un-normalized properties to calculate alignment efficiently
            f_norm = f.normal
            f_norm_len = f_norm.length
            
            tile_normal = mathutils.Vector(f[l_normal])
            tile_normal_len = tile_normal.length
            
            dot_val = f_norm.dot(tile_normal)
            denom = f_norm_len * tile_normal_len
            
            # Check normal alignment without executing extra memory clones
            normalized_dot = (dot_val / denom) if denom > 0.0 else 0.0
            
            is_top_f = normalized_dot > 0.9
            is_bot_f = normalized_dot < -0.9
    ...
    # Direct layer lookups to save dictionary hashing inside the edge loop
    layer_top = layers["top"]
    layer_bottom = layers["bottom"]
    layer_side = layers["side"]
    layer_width = layers["width"]
    layer_length = layers["length"]
    layer_boolean = layers.get("boolean")
    ...
    for edge in bm.edges:
        ...
        if orig_face_marker is not None:
            for f in linked:
                if face_cache[f][0]:  # index 0 is is_bool_f
                    is_boolean_edge = True
                    break

        is_top = False
        is_bot = False
        is_width_edge = False
        is_length_edge = False
        ...
        if not is_boolean_edge:
            if l_normal:
                for f in linked:
                    ...
                    if cache[1]:  # is_top_f
                        is_top = True
                    elif cache[2]:  # is_bot_f
                        is_bot = True

            if is_top and lu and lv:
                cap_face = None
                for f in linked:
                    if face_cache[f][1]:  # Find top face
                        cap_face = f
                        break
                        
                if cap_face is not None:
                    cache_cap = face_cache[cap_face]
                    axis_u = cache_cap[3]
                    axis_v = cache_cap[4]
                    
                    # Compute vector difference and normalize it cleanly exactly once
                    vec = edge.verts[0].co - edge.verts[1].co
                    vec_len = vec.length
                    if vec_len > 1e-5:
                        vec /= vec_len
        ...
        # Optimized direct assignments bypassing layer dict-key lookups
        edge[layer_top] = 1.0 if is_top else 0.0
        edge[layer_bottom] = 1.0 if is_bot else 0.0
        edge[layer_side] = 1.0 if (not is_boolean_edge and not (is_top or is_bot)) else 0.0
        edge[layer_width] = 1.0 if is_width_edge else 0.0
        edge[layer_length] = 1.0 if is_length_edge else 0.0
        if layer_boolean is not None:
            edge[layer_boolean] = 1.0 if is_boolean_edge else 0.0
...
def get_herringbone_matrices(groups, width, length, depth, rot_rad, row_offset, staggered_offset, max_random_offset,
    ...
    # Pre-calculate the gap displacement vector
    # This accounts for the gap on all sides by adjusting the placement 
    # of each tile relative to the herringbone junction.
    gap_u = length_gap / 2.0
    gap_v = width_gap / 2.0
    ...
    for key, data in groups.items():
        ...
        diag = math.sqrt(2)
        step_u = (length + length_gap) / diag
        step_v = (width + width_gap) / diag
        ...
        pad_x, pad_y = 2, 1
        row_min = math.floor(v_min / row_step) - pad_x - row_off_step
        row_max = math.ceil(v_max / row_step) + pad_x - row_off_step
        col_min = math.floor(u_min / col_step) - pad_y - col_off_step
        col_max = math.ceil(u_max / col_step) + pad_y - col_off_step
        ...
        for row in range(row_min, row_max):
            ...
            for col in range(col_min, col_max):
                ...
                # Apply gap-based centering for tile A
                # This ensures the gap exists between the tile edge and the grid junction
                center_a = pos_a + (gap_u * u_pos) + (gap_v * v_pos)
                all_matrices.append(create_tile_matrix(center_a, u_pos, v_pos, normal, length, width, depth) + (col, row))
                
                # Base position for tile B
                pos_b = pos_a + (step_u - step_v) * axis_u + (step_u - step_v) * axis_v
                
                # Apply gap-based centering for tile B
                center_b = pos_b + (gap_u * u_neg) + (gap_v * v_neg)
                all_matrices.append(create_tile_matrix(center_b, u_neg, v_neg, normal, length, width, depth) + (col, row))
...
def get_chevron_matrices(groups, width, length, depth, rot_rad, row_offset, staggered_offset, max_random_offset,
    ...
    for key, data in groups.items():
        ...
        pad_x = 1
        pad_y = 6
        row_min = math.floor(u_min / row_step) - pad_x - row_off_step
        row_max = math.ceil(u_max / row_step) + pad_x - row_off_step
        col_min = math.floor(v_min / leg_width) - pad_y - col_off_step
        col_max = math.ceil(v_max / leg_width) + pad_y - col_off_step
        ...
        for row in range(row_min, row_max):
            ...
            for col in range(col_min, col_max):
                ...
                # The col and row steps here perfectly handle the gaps between the atomic V-groups
                ...
                # FIXED: Added (length_gap / 2) * axis_u to the second leg origin calculation 
                # to maintain symmetry when length_gap is applied.
                leg_2_origin = origin + correction + (row_step / 2 + length_gap / 2) * axis_u
                mat, n, u, v = create_tile_matrix(leg_2_origin, u_neg, v_neg, normal, length, width, depth)
...
def get_windmill_matrices(groups, width, length, depth, rot_rad, row_offset, staggered_offset, max_random_offset,
    ...
    for key, data in groups.items():
        ...
        axis_u = (
            ...
            if abs(normal.dot(mathutils.Vector((0, 0, 1)))) < 0.9
        ...
        tile_length = length - length_gap
        tile_width = width - width_gap

        center_size = length - width - ((width_gap + length_gap) / 2.0)

        half_lgap = length_gap * 0.5
        half_wgap = width_gap * 0.5
        ...
        for row in range(row_min, row_max):
            ...
            for col in range(col_min, col_max):
                ...
                # Bottom
                p = origin + half_lgap * axis_u + half_wgap * axis_v
                all_matrices.append(create_tile_matrix(p, axis_u, axis_v, normal, tile_length, tile_width, depth) + (row, col))

                # Left
                p = origin + (length + width) * axis_u
                p += half_lgap * axis_v - half_wgap * axis_u
                all_matrices.append(create_tile_matrix(p, axis_v, -axis_u, normal, tile_length, tile_width, depth) + (row, col))

                # Right
                p = origin + width * axis_v + width * axis_u
                p += half_lgap * axis_v - half_wgap * axis_u
                all_matrices.append(create_tile_matrix(p, axis_v, -axis_u, normal, tile_length, tile_width, depth) + (row, col))

                # Top
                p = origin + length * axis_v + width * axis_u
                p += half_lgap * axis_u + half_wgap * axis_v
                all_matrices.append(create_tile_matrix(p, axis_u, axis_v, normal, tile_length, tile_width, depth) + (row, col))

                # Center
                p = origin + width * axis_v + width * axis_u + half_wgap * axis_u + half_lgap * axis_v
                all_matrices.append(create_tile_matrix(p, axis_u, axis_v, normal, center_size, center_size, depth) + (row, col))
                
    return all_matrices

def get_hexagon_matrices(groups, width, length, depth, rot_rad, row_offset, staggered_offset, max_random_offset,
                          width_gap, length_gap, offset_x, offset_y, offset_z, tiling_axis='BOTH'):
    """
    Pointy-top hexagon honeycomb tessellation. 'width' is the hexagon's
    flat-to-flat horizontal extent (across the two vertical flat sides),
    'length' is its point-to-point vertical extent -- these map directly
    onto the unit (-0.5..0.5) hex-prism template built in create_tile_batch
    (see _build_hex_prism_bmesh), the same way width/length map onto the
    unit cube for the other patterns. Alternating rows are shifted by half
    the horizontal pitch so the flat sides interlock, and rows are packed
    at 3/4 of the vertical pitch as is standard for hex grids.
    """
    all_matrices = []
    e_width, e_length = width - width_gap, length - length_gap

    u_step = width + width_gap
    v_step = (length + length_gap) * 0.75
    ...
    for key, data in groups.items():
        ...
        u_off_step = math.floor(offset_x / u_step) if u_step else 0
        v_off_step = math.floor(offset_y / v_step) if v_step else 0

        j_min = math.floor((v_min - offset_y) / v_step) - 2
        j_max = math.ceil((v_max - offset_y) / v_step) + 2

        for j in range(j_min, j_max):
            if tiling_axis == 'X' and (j + v_off_step) != 0:
                continue

            row_shift = (u_step * 0.5) if (j % 2 != 0) else 0.0
...
def get_basketweave_matrices(groups, width, length, depth, rot_rad, row_offset, staggered_offset, max_random_offset,
                              width_gap, length_gap, offset_x, offset_y, offset_z, tiling_axis='BOTH', tiles_per_weave=2):
    """
    Classic basket-weave pattern generator with corrected tiling axis filtering.
    """
    all_matrices = []
    n = max(1, int(tiles_per_weave))
    module_pitch = length + length_gap
    tile_long = length
    tile_short = max(0.0001, (length - (n - 1) * width_gap) / n)
    short_step = tile_short + width_gap
    ...
    for key, data in groups.items():
        ...
        anchor_base = next(v for face in faces for v in face)
        anchor = get_pattern_offset(anchor_base, axis_u, axis_v, normal, offset_x, offset_y, offset_z)
        ...
        col_min = math.floor((u_min - offset_x) / module_pitch) - 1
        col_max = math.ceil((u_max - offset_x) / module_pitch) + 1
        row_min = math.floor((v_min - offset_y) / module_pitch) - 1
        row_max = math.ceil((v_max - offset_y) / module_pitch) + 1

        for row in range(row_min, row_max):
            for col in range(col_min, col_max):
                # Apply Tiling Axis filter
                if tiling_axis == 'X' and col != 0: continue
                if tiling_axis == 'Y' and row != 0: continue

                module_origin = anchor + (col * module_pitch) * axis_u + (row * module_pitch) * axis_v

                if (col + row) % 2 == 0:
                    for k in range(n):
                        p = module_origin + (k * short_step) * axis_v
                        all_matrices.append(create_tile_matrix(p, axis_u, axis_v, normal, tile_long, tile_short, depth) + (col, row))
                else:
                    for k in range(n):
                        p = module_origin + (k * short_step) * axis_u
                        all_matrices.append(create_tile_matrix(p, axis_v, axis_u, normal, tile_long, tile_short, depth) + (col, row))
...
#    stretcher_rng = random.Random(random_offset_seed)
#    
#    # FIX: Calculate local dimensions from the bound_box and object scale.
#    # This ensures the footprint accurately matches the base mesh, ignoring any object-mode rotation bloating.
#    bbox = tile_obj.bound_box
#    local_x = max(v[0] for v in bbox) - min(v[0] for v in bbox)
#    local_y = max(v[1] for v in bbox) - min(v[1] for v in bbox)
#    
#    new_w = (local_x * tile_obj.scale.x) * width
#    new_l = (local_y * tile_obj.scale.y) * length
#    
#    # Failsafe against zero-division if a user selects an empty or 1D mesh
#    new_w = max(0.0001, new_w)
#    new_l = max(0.0001, new_l)
...
def _build_hex_prism_bmesh():
    """
    Builds a single pointy-top hexagonal prism as a fresh bmesh, spanning
    -0.5..0.5 on X and Y (and -0.5..0.5 on Z, matching the cube template's
    thickness convention). This plays the exact same role that
    bmesh.ops.create_cube() plays for the other patterns: a unit template
    that the per-tile placement matrix (see create_tile_matrix / the
    Diagonal scale in create_tile_batch) then stretches into the final
    tile width/length/depth. Used only for the HEXAGON pattern.
    """
    bm = bmesh.new()
    hx, hy = 0.5, 0.5
    # Pointy-top hex: points at top/bottom (+/-Y), flat vertical sides at +/-X.
    # Ordered counter-clockwise (viewed from +Z) so the top cap's face
    # normal comes out pointing +Z before recalc_face_normals fixes the rest.
    ring_xy = [
        (0.0, hy), (-hx, hy * 0.5), (-hx, -hy * 0.5),
        (0.0, -hy), (hx, -hy * 0.5), (hx, hy * 0.5),
    ]
    top_verts = [bm.verts.new((x, y, 0.5)) for x, y in ring_xy]
    bot_verts = [bm.verts.new((x, y, -0.5)) for x, y in ring_xy]
    ...
    for i in range(n):
        a, b = i, (i + 1) % n
        bm.faces.new([bot_verts[a], bot_verts[b], top_verts[b], top_verts[a]])
...
def get_placement_matrices(pattern, groups, width, length, depth, rot_rad, row_offset, staggered_offset, max_random_offset,
    ...
    if pattern == "HEXAGON":
        return get_hexagon_matrices(*args, tiling_axis=tiling_axis)
    if pattern == "BASKETWEAVE":
        return get_basketweave_matrices(*args, tiling_axis=tiling_axis, tiles_per_weave=tiles_per_weave)
    if pattern == "CUSTOM":
...
#    bool_mod.solver = 'MANIFOLD'  # Switched for massive performance gains on standard cuts
#    bool_mod.object = cutter_obj
#    bool_mod.use_self = False

#    # Evaluate the geometry directly via depsgraph instead of forcing bpy.ops view layer redraws
#    depsgraph = bpy.context.evaluated_depsgraph_get()
#    eval_obj = tmp_obj.evaluated_get(depsgraph)
#    clipped_mesh = bpy.data.meshes.new_from_object(eval_obj, preserve_all_data_layers=True, depsgraph=depsgraph)
...
def _clip_mesh_to_faces(mesh, faces, obj_matrix_world, depth, offset_z):
    """
    Optimized replacement for the boolean modifier. Uses BMesh bisect_plane 
    to clip tiles in memory, compatible with Blender 5.1+.
    """
    import bmesh
    # 1. Create BMesh from the mesh data
    bm = bmesh.new()
    bm.from_mesh(mesh)
    
    # 2. Derive plane data from the 'faces' input
    if faces and hasattr(faces, '__iter__'):
        # Get normal and a point from the first face to define the clipping plane
        normal = faces[0][0]
        # Calculate a point on the plane (using the first vertex of the first face)
        point = faces[0][1][0] 
        
        # 3. Perform bisection
        # Ensure 'geom' is passed as a list of mesh elements
...
def _extract_template_arrays(bm):
    """Flatten a template bmesh into numpy-friendly local-space data."""
    bm.verts.ensure_lookup_table()
    bm.faces.ensure_lookup_table()
    verts_local = np.array([v.co[:] for v in bm.verts], dtype=np.float64)  # (V,3)

    m_layer = bm.faces.layers.int.get("m_idx")
    if m_layer is None:
        # Shouldn't happen (layer is created earlier), but stay safe.
        m_layer = bm.faces.layers.int.new("m_idx")
        for f in bm.faces:
            f[m_layer] = f.material_index

    # Match original behavior: use the ACTIVE uv layer, not a name lookup.
    ...
    face_mat = []
    for f in bm.faces:
        faces_local.append(tuple(vert_index[v] for v in f.verts))
        face_uvs_local.append([tuple(loop[uv_layer].uv[:]) for loop in f.loops])
        face_mat.append(f[m_layer])

    return verts_local, faces_local, face_uvs_local, np.array(face_mat, dtype=np.int32)


def _stamp_tiles_batch(verts_local, top_mask, faces_local, face_uvs_local, face_mat_local,
                        mats, normals, depth_offsets, axis_us, axis_vs, cols, rows):
    """Vectorized: transform N tile instances of one template in a single pass."""
    N = mats.shape[0]
    ...
    world = np.einsum('nij,vj->nvi', rot, verts_local) + trans[:, None, :]  # (N,V,3)

    if np.any(top_mask):
        depth_vec = normals[:, None, :] * depth_offsets[:, None, None]  # (N,1,3)
        world[:, top_mask, :] += depth_vec

    verts_out = world.reshape(N * V, 3)

    faces_out = []
    for t in range(N):
        base = t * V
        for f in faces_local:
            faces_out.append(tuple(i + base for i in f))

    uv_out = []
    for _ in range(N):
        for f_uv in face_uvs_local:
            uv_out.extend(f_uv)

    F = len(faces_local)
    mat_idx_out = np.tile(face_mat_local, N)
    normal_out = np.repeat(normals, F, axis=0)
    axis_u_out = np.repeat(axis_us, F, axis=0)
    axis_v_out = np.repeat(axis_vs, F, axis=0)
    col_out = np.repeat(cols, F)
    row_out = np.repeat(rows, F)

    return verts_out, faces_out, uv_out, mat_idx_out, normal_out, axis_u_out, axis_v_out, col_out, row_out


def _filter_matrices_by_faces(matrices, faces, normal, tile_width, tile_length):
    """
    Pre-culls tile matrices that are far outside the boundaries of the target faces.
    Saves massive processing time by filtering out 'ghost' grid tiles in concave areas.
    """
    if not faces or not matrices:
        return matrices
        
    # Calculate a rough cull radius based on the tile size
    cull_radius = max(tile_width, tile_length) * 0.75
    
    # Establish local 2D projection axes from the first face
    v0 = faces[0][0]
    axis_u = (mathutils.Vector((0, 0, 1)) if abs(normal.dot(mathutils.Vector((0, 0, 1)))) < 0.9 else mathutils.Vector((0, 1, 0)))
    axis_u = (axis_u - axis_u.dot(normal) * normal).normalized()
    axis_v = normal.cross(axis_u).normalized()
    
    # Pre-calculate 2D bounding boxes for all target faces
    face_bounds = []
    for f in faces:
        projected_u = [v.dot(axis_u) for v in f]
        projected_v = [v.dot(axis_v) for v in f]
        
        min_u, max_u = min(projected_u), max(projected_u)
        min_v, max_v = min(projected_v), max(projected_v)
        
        # Expand bounds by the cull radius to allow tiles overlapping the edge
        face_bounds.append((
            min_u - cull_radius, max_u + cull_radius,
            min_v - cull_radius, max_v + cull_radius
        ))
        
    filtered_matrices = []
    
    # Check each tile against the face bounds
    for mat_data in matrices:
        # mat_data: (Matrix, normal, axis_u, axis_v, col, row)
        center = mat_data[0].translation
        u_coord = center.dot(axis_u)
        v_coord = center.dot(axis_v)
        
        keep = False
        for (min_u, max_u, min_v, max_v) in face_bounds:
            if min_u <= u_coord <= max_u and min_v <= v_coord <= max_v:
                keep = True
                break
                
        if keep:
            filtered_matrices.append(mat_data)
...
def create_tile_batch(face_data, width, length, depth, rotation_angle, row_offset, staggered_offset, max_random_offset,
                       ...
                       use_boolean_clip=True, tiles_per_weave=2):
    ...
    for flip_u in u_options:
        for flip_v in v_options:
            ...
            if custom_src_obj is not None:
                bm.from_mesh(custom_src_obj.data)
                scale_only = mathutils.Matrix.Diagonal(custom_src_obj.scale).to_4x4()
                bm.transform(scale_only)
            elif pattern == "HEXAGON":
                bm.free()
                bm = _build_hex_prism_bmesh()
            else:
    ...
    # --- Precompute flat numpy arrays + top_mask per template key ---
    template_arrays = {}
    template_top_mask = {}
    for key, t_bm in templates.items():
        verts_local, faces_local, face_uvs_local, face_mat_local = _extract_template_arrays(t_bm)
        zs = verts_local[:, 2]
        z_mid = (zs.min() + zs.max()) * 0.5
        template_arrays[key] = (verts_local, faces_local, face_uvs_local, face_mat_local)
        template_top_mask[key] = zs > z_mid
    ...
    m_base_mat = master_bm.faces.layers.int.new("base_m_idx")
    ...
    for group_key, group_val in groups.items():
        # Get raw grid matrices
        raw_matrices = get_placement_matrices(
            pattern, {group_key: group_val}, width, length, depth, rot_rad, row_offset, staggered_offset, max_random_offset,
            width_gap, length_gap, offset_x, offset_y, offset_z, random_offset_seed, random_depth, random_depth_seed,
            custom_obj_name=custom_obj_name, tiling_axis=tiling_axis, tiles_per_weave=tiles_per_weave
        )

        # Apply pre-culling to drop instances floating in empty space
        group_matrices = _filter_matrices_by_faces(
            raw_matrices, 
            group_val["faces"], 
            group_val["normal"], 
            width + width_gap, 
        ...
        # Sequential pass: only the RNG-dependent bits, must preserve exact call order.
        tiles_by_key = {}
        for final_mat, normal, axis_u, axis_v, tile_col_val, tile_row_val in group_matrices:
            ...
            bucket = tiles_by_key.setdefault((f_u, f_v), {
                "mats": [], "normals": [], "depth_offsets": [],
                "axis_u": [], "axis_v": [], "cols": [], "rows": []
            })
            bucket["mats"].append([list(row) for row in final_mat])
            bucket["normals"].append(normal)
            bucket["depth_offsets"].append(tile_depth_offset)
            bucket["axis_u"].append(axis_u)
            bucket["axis_v"].append(axis_v)
            bucket["cols"].append(tile_col_val)
            bucket["rows"].append(tile_row_val)

        all_verts, all_faces, all_uvs = [], [], []
        all_mat_idx, all_normal, all_axis_u, all_axis_v, all_col, all_row = [], [], [], [], [], []
        vert_offset = 0

        for key, bucket in tiles_by_key.items():
            verts_local, faces_local, face_uvs_local, face_mat_local = template_arrays[key]
            top_mask = template_top_mask[key]

            mats = np.array(bucket["mats"], dtype=np.float64)
            normals = np.array(bucket["normals"], dtype=np.float64)
            depth_offsets = np.array(bucket["depth_offsets"], dtype=np.float64)
            axis_us = np.array(bucket["axis_u"], dtype=np.float64)
            axis_vs = np.array(bucket["axis_v"], dtype=np.float64)
            cols = np.array(bucket["cols"], dtype=np.float64)
            rows = np.array(bucket["rows"], dtype=np.float64)

            (verts_out, faces_out, uv_out, mat_idx_out,
             normal_out, axis_u_out, axis_v_out, col_out, row_out) = _stamp_tiles_batch(
                verts_local, top_mask, faces_local, face_uvs_local, face_mat_local,
                mats, normals, depth_offsets, axis_us, axis_vs, cols, rows
            )

            faces_out = [tuple(i + vert_offset for i in f) for f in faces_out]
            vert_offset += verts_out.shape[0]

            all_verts.append(verts_out)
            all_faces.extend(faces_out)
            all_uvs.extend(uv_out)
            all_mat_idx.append(mat_idx_out)
            all_normal.append(normal_out)
            all_axis_u.append(axis_u_out)
            all_axis_v.append(axis_v_out)
            all_col.append(col_out)
            all_row.append(row_out)

        # Build the group mesh (empty-group-safe)
        verts_arr = np.concatenate(all_verts, axis=0) if all_verts else np.zeros((0, 3))
        mat_idx_arr = np.concatenate(all_mat_idx, axis=0) if all_mat_idx else np.zeros((0,), dtype=np.int32)
        normal_arr = np.concatenate(all_normal, axis=0) if all_normal else np.zeros((0, 3))
        axis_u_arr = np.concatenate(all_axis_u, axis=0) if all_axis_u else np.zeros((0, 3))
        axis_v_arr = np.concatenate(all_axis_v, axis=0) if all_axis_v else np.zeros((0, 3))
        col_arr = np.concatenate(all_col, axis=0) if all_col else np.zeros((0,))
        row_arr = np.concatenate(all_row, axis=0) if all_row else np.zeros((0,))

        group_mesh = bpy.data.meshes.new("_tile_group_tmp")
        group_mesh.from_pydata(verts_arr.tolist(), [], all_faces)
        group_mesh.update(calc_edges=True)

        uv_layer = group_mesh.uv_layers.new(name="UVMap")
        uv_flat = np.array(all_uvs, dtype=np.float64).reshape(-1) if all_uvs else np.zeros((0,))
        uv_layer.data.foreach_set("uv", uv_flat)

        group_mesh.polygons.foreach_set("material_index", mat_idx_arr.astype(np.int32))

        def _add_face_attr(name, dtype, data):
            attr = group_mesh.attributes.new(name=name, type=dtype, domain='FACE')
            if dtype == 'FLOAT_VECTOR':
                attr.data.foreach_set("vector", data.reshape(-1))
            else:
                attr.data.foreach_set("value", data)

        _add_face_attr("tile_normal", 'FLOAT_VECTOR', normal_arr)
        _add_face_attr("tile_axis_u", 'FLOAT_VECTOR', axis_u_arr)
        _add_face_attr("tile_axis_v", 'FLOAT_VECTOR', axis_v_arr)
        _add_face_attr("tile_col", 'FLOAT', col_arr)
        _add_face_attr("tile_row", 'FLOAT', row_arr)
        _add_face_attr("base_m_idx", 'INT', mat_idx_arr.astype(np.int32))
        _add_face_attr("_orig_tile_face", 'FLOAT', np.ones(len(all_faces), dtype=np.float64))

        if use_boolean_clip:
#            clipped_mesh = _clip_mesh_to_faces(group_mesh, group_val["faces"], depth, offset_z)
...
def _tile_matches_pattern(col, row, mode, step_x, offset_x, step_y, offset_y, invert, sep_x, sep_y):
    """ Shared match test for 'is this real tile (identified by its col/row grid index) picked by this pattern'. """
    # Prevent division by zero if step is invalid
    if (step_x + sep_x) == 0 or (step_y + sep_y) == 0:
        return False
        
    if mode == 'CHECKER':
        # Calculate position relative to the step and gap
        effective_x = (col - offset_x)
        effective_y = (row - offset_y)
        
        # Check if we are inside a gap
        if (effective_x % (step_x + sep_x) >= step_x) or (effective_y % (step_y + sep_y) >= step_y):
            return False
            
        block_col = math.floor(effective_x / (step_x + sep_x))
        block_row = math.floor(effective_y / (step_y + sep_y))
        
        parity = (block_col + block_row) % 2
        return (parity == 1) if invert else (parity == 0)
        
    return ((col - offset_x) % (step_x + sep_x) == 0) and ((row - offset_y) % (step_y + sep_y) == 0)
...
def _apply_material_rules(batch_obj):
    ...
    # True per-face base material index, remembered at generation time
    # (e.g. transferred per-face from a CUSTOM tile template). Falls back
    # to None for meshes generated before this layer existed.
    l_base_mat = bm.faces.layers.int.get("base_m_idx")

    for f in bm.faces:
        # Reset to the face's REAL base material (not a hardcoded 0) so:
        #  - a face that doesn't match any rule keeps its original/custom
        #    material instead of being forced onto slot 0, and
        #  - live-editing a rule's step/offset/material still updates
        #    correctly, since a face that previously matched an
        #    now-changed rule properly falls back to its base material
        #    instead of staying stuck on the old match.
        f.material_index = f[l_base_mat] if l_base_mat is not None else 0
...
#            if _tile_matches_pattern(col, row, rule.mode, rule.step_x, rule.offset_x, rule.step_y, rule.offset_y, rule.invert):
            if _tile_matches_pattern(col, row, rule.mode, rule.step_x, rule.offset_x, rule.step_y, rule.offset_y, rule.invert, rule.sep_x, rule.sep_y):
    ...
    # Tag the object to ensure modifiers (like BooleanClip) and the viewport update immediately
    batch_obj.update_tag()
...
PATTERN_ITEMS = [
    ...
    ('HEXAGON', "Hexagon", "Hexagonal honeycomb pattern"),
    ('BASKETWEAVE', "Basket Weave", "Woven pairs of rectangular tiles"),
...
def update_pattern_defaults(self, context):
    ...
    if pattern == 'CUSTOM':
        ...
        self.max_random_offset = 0.0
        self.random_offset_seed = 0
        self.width_gap = 0.005
        self.length_gap = 0.005
        self.uv_random_seed = 77
        self.flip_mode = 'BOTH'
        self.random_depth = 0.0
        self.random_depth_seed = 42
        self.offset_x = 0.0
...
class SmartTileMaterialRule(PropertyGroup):
    ...
    sep_x: IntProperty(name="Separation X", default=0, update=_trigger_material_rule_update)
    sep_y: IntProperty(name="Separation Y", default=0, update=_trigger_material_rule_update)
...
class SmartTileSettings(PropertyGroup):
    ...
    tiles_per_weave: IntProperty(
        name="Tiles Per Weave",
        default=2,
        min=1,
        description="How many parallel strips make up each group in the Basket Weave pattern (2 = classic look)",
        update=_trigger_realtime_update
    )
    ...
    flip_mode: EnumProperty(items=FLIP_ITEMS, name="UV Flip Mode", default='BOTH', update=_trigger_realtime_update)
    uv_random_seed: IntProperty(name="UV Random Seed", default=235, update=_trigger_realtime_update)
    ...
    show_edge_attrs: bpy.props.BoolProperty(
        name="Edge Attribute Names",
        description="Show/Hide edge attribute settings",
        default=False
    )
    
    show_material_rules: bpy.props.BoolProperty(
        name="Show Material Rules",
        default=False
    )
    ...
    def execute(self, context):
        ...
        for obj in selected_meshes:
            ...
            me_batch, uv_warning = create_tile_batch(
                ...
                tiles_per_weave=settings.tiles_per_weave
            )
            if uv_warning:
...
class SMARTTILE_PT_panel(Panel):
    ...
    def draw(self, context):
        ...
        if settings.pattern != 'BASKETWEAVE':
            col.prop(settings, "width")
        col.prop(settings, "length")
        col.prop(settings, "depth")
        if settings.pattern == 'BASKETWEAVE':
            col.label(text="Strip width = Length / Tiles Per Weave", icon='INFO')
        ...
        if settings.pattern == 'BASKETWEAVE':
            layout.prop(settings, "tiles_per_weave")
        ...
        # --- Collapsible Edge Attribute Section ---
        box = layout.box()
        col = box.column()
        icon = 'TRIA_DOWN' if settings.show_edge_attrs else 'TRIA_RIGHT'
        col.prop(settings, "show_edge_attrs", text="Edge Attribute Names", icon=icon, emboss=False)
        
        if settings.show_edge_attrs:
            sub = col.column()
            sub.prop(settings, "attr_bevel_top")
            sub.prop(settings, "attr_bevel_bottom")
            sub.prop(settings, "attr_bevel_side")
            sub.prop(settings, "attr_width_edge")
            sub.prop(settings, "attr_length_edge")
            sub.prop(settings, "attr_boolean_edge")

        # --- Collapsible Material Rules Section ---
        if is_result:
            box = layout.box()
            col = box.column()
            icon = 'TRIA_DOWN' if settings.show_material_rules else 'TRIA_RIGHT'
            col.prop(settings, "show_material_rules", text="Material Rules (persists on regen)", icon=icon, emboss=False)

            if settings.show_material_rules:
                row = col.row()
                row.template_list(
                ...
                r_col.operator("object.smart_tile_add_material_rule", text="", icon='ADD')
                r_col.operator("object.smart_tile_remove_material_rule", text="", icon='REMOVE')
                ...
                if settings.material_rules:
                    ...
                    if active_rule.mode == 'CHECKER':
                        r = sub.row(align=True)
                        r.prop(active_rule, "sep_x", text="Gap X")
                        r.prop(active_rule, "sep_y", text="Gap Y")    
                        sub.prop(active_rule, "invert")
                
                col.operator("object.smart_tile_apply_material_rules", icon='FILE_REFRESH')
